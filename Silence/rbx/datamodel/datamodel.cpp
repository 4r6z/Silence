...
std::uint64_t silence::utils::datamodel::get_game_address()
{
    ...
    if (rbx_log_data.find(("UGCGameController::finishTeleportWithJoinScriptPayload")) != std::string::npos)
    {
        size_t posTeleport = rbx_log_data.find(("start dataModel("), rbx_log_data.find(("UGCGameController::finishTeleportWithJoinScriptPayload")));
    ...
    size_t posMain = rbx_log_data.find(("initialized DataModel("));
